...
import com.example.ibesteeth.git_keybordtest.Util.TDevice;
...
//            editText.append(displayEmoji(editText.getResources(),emojicon.getRemote()));
            ...
            editText.setText(text + getEmijoByUnicode(emojicon.getResId()));
//            editText.append(getEmijoByUnicode(emojicon.getResId()));
        } else {
            // 将已选中的部分替换为表情(当长按文字时会多选刷中很多文字)
//            Spannable str = displayEmoji(editText.getResources(),emojicon.getRemote());
//            editText.getText().replace(Math.min(start, end),Math.max(start, end), str, 0, str.length());

            String str=emojicon.getEmojiStr();
            editText.getText().replace(Math.min(start, end),Math.max(start, end), str, 0, str.length());
        }
    }
